...
综述稿件宜含检索策略、PRISMA 或主题框架等图示：
- **结构化/可数据化图**（如简单计数、阶段统计）：可用 **matplotlib / seaborn** skill。
- **流程图、概念框架、机制示意**：用项目 **`AGENTS.md`** 中**豆包 Seedream** 预置脚本。
本镜像不包含 OpenRouter `scientific-schematics`。
