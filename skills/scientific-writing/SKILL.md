...
description: Write scientific manuscripts in full paragraphs (never bullet points). Two-stage process: (1) section outlines with key points using citation-management, literature-review, and model-assisted lookup, then (2) convert to flowing prose. IMRAD structure, citations (APA/AMA/Vancouver), figures/tables, reporting guidelines (CONSORT/STROBE/PRISMA), for research papers and journal submissions.
...
This skill combines structured literature handling with well-formatted written outputs. Prefer **citation-management** and **literature-review** plus the conversation model for search, outlines, and citation verification (this deployment does not use external Parallel/OpenRouter research APIs).

Scientific writing is a process for communicating research with precision and clarity. Write manuscripts using IMRAD structure, citations (APA/AMA/Vancouver), figures/tables, and reporting guidelines (CONSORT/STROBE/PRISMA). Apply this skill for research papers and journal submissions.

**Critical Principle: Always write in full paragraphs with flowing prose. Never submit bullet points in the final manuscript.** Use a two-stage process: first create section outlines with key points using **citation-management** / **literature-review** and the model, then convert those outlines into complete paragraphs.
...
## Figures and Visuals (LinkMed / 本项目)

- **有数据的统计图**（组间比较、趋势、分布、相关、热图等）：使用内置 **matplotlib** / **seaborn** / **scientific-visualization** skill（必要时 **statsmodels**）。遵守项目根目录 **`AGENTS.md`**：不在用户可见输出中回显保存路径。
- **概念图、流程图、机制示意图、Graphical Abstract（概念插画类）**：使用 **`AGENTS.md`** 中的**豆包 Seedream 预置脚本**；本镜像不包含 OpenRouter `scientific-schematics`。
- **作图原则**：能用**数据复现**的用 Python 绘图；仅有文字描述、无结构化数据时用豆包。综合稿件宜包含方法与结果类图，并按期刊需要准备图示摘要等（工具链如上）。
...
1. Use **citation-management**、**literature-review** and the model to gather relevant literature and data
...
**citation-management**、**literature-review** and the model are essential for Stage 1 (creating outlines):
1. Search for relevant papers using those tools and the model
...
2. For each section below, follow the two-stage process:
   - **First**: Create outline with bullet points using **citation-management**、**literature-review** and the model
